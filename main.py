...
def parse_arguments() -> argparse.Namespace:
    parser = argparse.ArgumentParser(prog="aegis-api", description="Aegis-API — Unified API Security Scanner (REST | SOAP | GraphQL)")
    parser.add_argument("-t", "--target", required=True, help="Base URL of the target API")
    parser.add_argument("-a", "--auth", required=False, default=None, help="JWT token for authentication (omit 'Bearer ' prefix)")
    parser.add_argument("--protocol", required=False, default="auto", choices=["auto", "rest", "soap", "graphql"], help="API protocol to scan")
    parser.add_argument("-w", "--wordlist", required=False, help="Path to ID wordlist for IDOR testing")
    parser.add_argument("-p", "--params", required=False, help="Path to parameter wordlist for mass assignment testing")
    parser.add_argument("--proxy", required=False, default="http://127.0.0.1:8080", help="Proxy URL for traffic interception")
    parser.add_argument("--no-proxy", action="store_true", help="Disable proxy routing entirely")
    parser.add_argument("--ci-mode", action="store_true", help="Enable CI/CD mode")
    parser.add_argument("--fail-threshold", required=False, default="high", choices=["critical", "high", "medium", "low"], help="Minimum severity level that causes the pipeline to fail")
    return parser.parse_args()

def build_session(jwt_token: str | None, proxy_url: str | None = None) -> requests.Session:
    session = requests.Session()
    headers = {"Content-Type": "application/json", "User-Agent": "AegisAPI/2.0"}
    ...
    retry_strategy = Retry(total=3, backoff_factor=1, status_forcelist=[500, 502, 503, 504], allowed_methods=["HEAD", "GET", "PUT", "POST", "DELETE", "OPTIONS"])
...
def append_to_audit_log(filename: str = "audit_log.csv", timestamp: str = None, method: str = None, url: str = None, status_code: int = None, payload: str = None) -> None:
    file_exists = os.path.isfile(filename)
    row = [timestamp or datetime.now().strftime("%Y-%m-%d %H:%M:%S"), method or "N/A", url or "N/A", str(status_code) if status_code is not None else "N/A", str(payload) if payload else "None"]
...
def scan_mass_assignment(session: requests.Session, target_url: str, endpoint: str, base_payload: dict, param_wordlist: list) -> list:
    ...
    for param in param_wordlist:
        ...
        try:
            ...
            append_to_audit_log(method="PUT", url=url, status_code=response.status_code, payload=f"injected_param={param}")

            if response.status_code in [200, 201]:
                print(f"[!] Potential mass assignment: server accepted '{param}' (status {response.status_code})")
                findings.append({
...
def run_rest_scan(args: argparse.Namespace) -> list:
    ...
    swagger_findings, idor_targets, mass_targets = _run_swagger_discovery(session, args.target)

    print("\n[*] REST Parameter Discovery")
    auto_rest_params = discover_rest_params(session=session, base_url=args.target, mass_targets=mass_targets)

    final_param_wordlist = build_param_candidate_list(discovered=auto_rest_params, wordlist=param_wordlist, override=bool(args.params))

    print(f"[+] Parameter candidates: {len(final_param_wordlist)} ({'user override' if args.params else 'auto-discovered + defaults'})")
    ...
    for target in mass_targets:
        ...
        results = scan_mass_assignment(session, args.target, target["path"], base_payload, final_param_wordlist)
...
def run_soap_scan(args: argparse.Namespace) -> list:
    ...
    print("\n[*] SOAP Parameter Discovery (WSDL input elements)")
    ...
    print(f"[+] SOAP parameter candidates: {len(final_soap_params)} ({'user override' if args.params else 'auto-discovered + defaults'})")
...
def run_graphql_scan(args: argparse.Namespace) -> list:
    ...
    if args.auth:
        ...
        print("\n[*] JWT Analysis skipped (no --auth token provided)")
    ...
    print(f"[*] Introspection scan complete — {len(introspection_findings)} finding(s)")

    schema = get_schema_for_other_modules(args.target, session)

    print("\n[*] GraphQL Parameter Discovery (INPUT_OBJECT arguments)")
    ...
    print(f"[+] GraphQL parameter candidates: {len(final_graphql_params)} ({'user override' if args.params else 'auto-discovered + defaults'})")
    ...
    print(f"[*] Depth limit scan complete — {len(depth_findings)} finding(s)")
    ...
    print(f"[*] Field auth scan complete — {len(field_findings)} finding(s)")
    ...
    print(f"[*] Batch abuse scan complete — {len(batch_findings)} finding(s)")
...
class VaptReport(FPDF):
    def header(self):
        ...
        self.cell(0, 10, "Aegis-API Unified Security Assessment Report", border=0, ln=1, align="C")
        self.ln(5)
...
def generate_pdf_report(findings: list, output_path: str = "reports/VAPT_Report.pdf") -> None:
...
def save_evidence_file(findings: list, output_path: str = "reports/evidence.txt") -> None:
    ...
    try:
        with open(output_path, "w", encoding="utf-8") as f:
            f.write("=" * 60 + "\n")
            f.write(f"Aegis-API Evidence File — {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}\n")
            f.write("=" * 60 + "\n\n")
